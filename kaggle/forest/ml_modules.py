...
def test_regression_split(dataframe):

    '''
    Creates a seperate regression for outcomes 1 and 2
    '''

    print '-----> Perfroming Data Accuracy tests on split regression...'

    # keep the original dataframe
    dataframe_old = dataframe.copy()

    # add a new outcome column/; new outcomes are '1or2' and 'rest'( 0 or 1)
    dataframe['binary_target'] = 1

    for row in range(len(dataframe)):
        
        if dataframe['Cover_Type'][row] == 1 or dataframe['Cover_Type'][row] == 2:
            dataframe['binary_target'][row] = 1
        
        else: dataframe['binary_target'][row] = 2    
    ...
    dataframe = set_column_sequence(dataframe, ['binary_target'])
    ...
    binary_forest = algorithm(n_estimators = 1000,
                           max_features = 'auto',
                           min_samples_split = 1,
                           max_depth = 5,
                           learning_rate = 0.3
    ...
    # train the forest
    binary_forest = binary_forest.fit(train_data_x, train_data_y)
    ...
    for x in estimator_list:
        ...
        print '-----> Training with ', x, 'trees...'

        binary_forest = algorithm(n_estimators = x,
                           max_features = 'auto',
                           min_samples_split = 1,
                           max_depth = 5,
                           learning_rate = 0.5
        ...
        # Fit the data
        binary_forest = forest.fit(train_data_x, train_data_y)
     
        test_accuracy.append(sklearn.metrics.accuracy_score(test_data_y, binary_forest.predict(test_data_x) ))
        train_accuracy.append(sklearn.metrics.accuracy_score(train_data_y, binary_forest.predict(train_data_x) ))
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig('crossVal/binary_split_test_train_error_'+title+'.pdf')
    '''

    # ==================================
    # Now that we have the binary split trained we need to create two regressions: one for class 1 and 2 and one for all else.
    # two new dataframes are needed for classes 1 and 2 and all esle

    df_12 = dataframe_old[(dataframe_old['Cover_Type']==1) | (dataframe_old['Cover_Type']==2)] # dataframe_old['Cover_Type']==2]
    
    df_rest = dataframe_old[(dataframe_old['Cover_Type'] != 1 ) | (dataframe_old['Cover_Type'] != 2)]
    
    #print df_12.head(30)
    #print df_rest.head(30)

    # move known outcomes to the front
    df_12 = set_column_sequence(df_12, ['Cover_Type'])
    df_rest = set_column_sequence(df_rest, ['Cover_Type'])
    
    # convert pandas to numpy array
    d_12 = df_12.values
    d_rest = df_rest.values
    
    # split training set into a test and train part.  80% train, 20% Test
    train_data_12, test_data_12 = cross_validation.train_test_split(d_12, test_size=0.4, random_state=42)

    train_data_rest, test_data_rest = cross_validation.train_test_split(d_rest, test_size=0.4, random_state=42)

    # Now split the data into x(features) and y(known outcomes)
    train_data_12_x = train_data_12[:, 1:]
    train_data_12_y = train_data_12[:, 0]
    
    train_data_rest_x = train_data_rest[:, 1:]
    train_data_rest_y = train_data_rest[:, 0]
    
    # Now make the BDTs for each dataframe
    algorithm = GradientBoostingClassifier; title = 'GradientBoostingClassifier'

    forest_12 = algorithm(n_estimators = 1000,
                          max_features = 'auto',
                          min_samples_split = 1,
                          max_depth = 5,
                          learning_rate = 0.3
    ...
    # train the forest
    forest_12 = forest_12.fit(train_data_12_x, train_data_12_y)    
    
    
    # now onto the rest
    forest_rest = algorithm(n_estimators = 1000,
                            max_features = 'auto',
                            min_samples_split = 1,
                            max_depth = 5,
                            learning_rate = 0.3
    ...
    # train the forest
    forest_rest = forest_rest.fit(train_data_rest_x, train_data_rest_y)
    
    # ========= Seperate BDTs have been made ============
    #  Now apply the two BDTs to the full trainingset after checking for which cover typw with the binary BDT
    # remove old outcomes
    dataframe_old = dataframe_old.drop(['Cover_Type'], axis=1)
    dataset1 = dataframe_old.values
    
     # split training set into a test and train part.  80% train, 20% Test
    train_data1, test_data1 = cross_validation.train_test_split(dataset1, test_size=0.4, random_state=42)
    
    # Now split the data into x(features) and y(known outcomes)
    train_data1_x = train_data1[:, 1:]
    train_data1_y = train_data1[:, 0]
    
    test_data1_x = test_data1[:, 1:]
    test_data1_y = test_data1[:, 0]
    
    # predict the binary split
    binary_forest.predict(test_data1_x)
    
    # based on the results of binary_forest, send the events to either forest_12 or forest_rest
    if  
    test_accuracy.append(sklearn.metrics.accuracy_score(test_data_y, binary_forest.predict(test_data_x) ))    

    
# end test_regression_split



def num_data_accuracy(dataframe):
    ...
    data_precent = [dataframe.shape[0]*x for x in [.01, .1, .2, .3, .4, .5, .6, .7, .8, .9, 1]]

    test_accuracy  = []
    train_accuracy = []

    for x in data_precent:
    
        # randomly select x events from dataset
        data_skimmed = dataframe.ix[np.random.choice(dataframe.index, x)]

        # convert pandas to numpy array
        dataset = data_skimmed.values
    ...
    # end data percent loop
    
    # fill accuracy plots
    plt.plot(data_precent, test_accuracy, 'ro', label='test')
    plt.plot(estimator_list, train_accuracy, 'bo', label='train')
    #plt.plot(estimator_list, random_accuracy, 'yo', label='random')
    #plt.plot(estimator_list, most_freq_accuracy, 'go', label='most frequent')
    plt.title('How Much Data Should I Use')
    plt.xlabel('DataSet Percentage')
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig('crossVal/data_percent_accuracy.pdf')
